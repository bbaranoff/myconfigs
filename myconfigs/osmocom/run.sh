...
# PHY_MODE=qemu              : fake_trx → QEMU Calypso L1 → l1ctl_bridge → mobile
#
set -euo pipefail
...
init_tmux() {
    ...
    sleep 1
    rm -f /tmp/osmocom_l2* /tmp/qemu-calypso-mon*.sock "$TMUX_SOCKET"
...
if [ "$PHY_MODE" = "virtphy" ]; then
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # MODE QEMU : fake_trx → QEMU Calypso L1 (L1CTL socket intégré) → mobile
    #
    #   mobile ↔ QEMU (Calypso L1 + L1CTL unix sock) ↔ (TRXD UDP) ↔ fake_trx ↔ osmo-bts-trx ↔ BSC
    #
    # QEMU exécute le vrai firmware L1 Calypso et expose directement
    # le socket L1CTL vers mobile (pas de bridge externe).
    # ─────────────────────────────────────────────────────────────────────────

    echo -e "${GREEN}=== [4/10] FakeTRX + BTS ===${NC}"
    ...
    echo -e "${GREEN}=== [5/10] QEMU MS (${N_MS} instances Calypso L1) ===${NC}"
    for ms in $(seq 1 "$N_MS"); do
        local_l2=$(l2_sock "$ms")
        local_port=$(bb_port "$ms")
        mon_sock="/tmp/qemu-calypso-mon-ms${ms}.sock"

        cmd="CALYPSO_TRX_PORT=${local_port} L1CTL_SOCK=${local_l2} ${QEMU_BIN} -M calypso -cpu arm946 -display none -serial pty -serial pty -monitor unix:${mon_sock},server,nowait -kernel ${L1_FW}"
        ...
        if [ "$pl" -eq 0 ]; then
            ...
            bash -c "$cmd" >"/var/log/osmocom/qemu_ms${ms}.log" 2>&1 &
        fi
        echo -e "  ${CYAN}[ue${ms}]${NC} l2=${local_l2} trx=${local_port}"
    done

    # Laisser les QEMU démarrer, puis envoyer 'cont' pour lancer les CPU
    sleep 3
    for ms in $(seq 1 "$N_MS"); do
        mon_sock="/tmp/qemu-calypso-mon-ms${ms}.sock"
    ...
    # Vérifier que les sockets L2 sont créés
    for ms in $(seq 1 "$N_MS"); do
        local_l2=$(l2_sock "$ms")
        if [ -S "$local_l2" ]; then
            echo -e "  ${GREEN}✓${NC} ue${ms} socket ${local_l2}"
        else
            echo -e "  ${RED}✗${NC} ue${ms} socket ${local_l2} manquant"
        fi
    done
